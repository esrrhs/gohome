	...
	run := func(hashSpecs []hashSpec) bool {
		...
		for _, v := range hashSpecs {
			...
			if hex.EncodeToString(result) != v.output {
				loggo.Error("%v need %v get %v", v.variant, v.output, hex.EncodeToString(result))
				return false
			}
		}
		return true
		...
		for _, v := range hashSpecs {
			...
			for j, _ := range result {
				if result[j] != v.output[j] {
					loggo.Error("%v need %v get %v", v.variant, hex.EncodeToString(v.output[0:len(result)]), hex.EncodeToString(result))
					return false
				}
			}
		}
