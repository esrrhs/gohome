package aes

import (
	"fmt"
	"testing"

	"github.com/esrrhs/gohome/common"
	"github.com/esrrhs/gohome/crypto/cryptonight/inter/sha3"
)

type TestCryptoNightBe struct {
	// DO NOT change the order of these fields in this struct!
	// They are carefully placed in this order to keep at least 16-byte aligned
	// for some fields.
	//
	// In the future the alignment may be set explicitly, see
	// https://github.com/golang/go/issues/19057

	scratchpad [4 * 1024 * 1024 / 8]uint64 // 4 MiB scratchpad for memhard loop
	finalState [25]uint64                  // state of keccak1600
	_          [8]byte                     // padded to keep 16-byte align (0x2000d0)

	blocks [16]uint64 // temporary chunk/pointer of data
	rkeys  [40]uint32 // 10 rounds, instead of 14 as in standard AES-256
}

var cn1_be TestCryptoNightBe
var cn2_be TestCryptoNightBe

func Test0000Be(t *testing.T) {
	common.DebugSetBigEndian(true)
	defer common.DebugResetBigEndian()

	data := []byte{}
	sha3.Keccak1600State(&cn1_be.finalState, data[0:76])
	sha3.Keccak1600State(&cn2_be.finalState, data[0:76])
	CnExpandKeyGo(cn1_be.finalState[:4], &cn1_be.rkeys)
	CnExpandKeyGoSoft(cn2_be.finalState[:4], &cn2_be.rkeys)
	copy(cn1_be.blocks[:], cn1_be.finalState[8:24])
	copy(cn2_be.blocks[:], cn2_be.finalState[8:24])

	for i := 0; i < 2*1024*1024/8; i += 16 {
		for j := 0; j < 16; j += 2 {
			CnRoundsGo(cn1_be.blocks[j:j+2], cn1_be.blocks[j:j+2], &cn1_be.rkeys)
			CnRoundsGoSoft(cn2_be.blocks[j:j+2], cn2_be.blocks[j:j+2], &cn2_be.rkeys)

			if cn1_be.blocks[j] != cn2_be.blocks[j] || cn1_be.blocks[j+1] != cn2_be.blocks[j+1] {
				t.Errorf("diff at %v", i)
				return
			}
		}
	}
}

var cn_be TestCryptoNight

func Test0001Be(t *testing.T) {
	common.DebugSetBigEndian(true)
	defer common.DebugResetBigEndian()

	var a1 [2]uint64
	var c1 [2]uint64
	var a2 [2]uint64
	var c2 [2]uint64
	for i := 0; i < 2; i++ {
		a1[i] = uint64(i)
		a2[i] = uint64(i)
		c1[i] = uint64(i)
		c2[i] = uint64(i)
	}
	CnSingleRoundGo(c1[:2], cn_be.scratchpad[0:2], &a1)
	CnSingleRoundGoSoft(c2[:2], cn_be.scratchpad[0:2], &a2)

	if c1[0] != c2[0] || c1[1] != c2[1] {
		t.Errorf("diff at ")
		return
	}
	fmt.Println(c1)
	fmt.Println(c2)
}
